...
let flow_lines_portfolio = function(f) {
  let point1, point2, targetPoint;
  let resetNum = 0;
  let easing1 = 0.06;
  let easing2 = 0.03;
  ...
  f.setup = () => {
    let cnv = f.createCanvas(laep_parent.clientWidth, laep_parent.clientHeight);

    cnv.id('lae-portfolio');

    point1 = f.createVector(f.width/2, f.height/2);
    point2 = f.createVector(f.width/2, f.height/2);
    targetPoint = f.createVector(f.width/2, f.height/2);

    col = [f.random(255), f.random(255), f.random(255), f.random(20)];
...
let lae_parent = document.getElementById('hero');
let p5_flowlines = new p5(flow_lines, 'hero');

let laep_parent = document.getElementById('lae');
let p5_flowlines_portfolio = new p5(flow_lines_portfolio, 'lae');
